...
def propose_strategies(yf_obj, defined_risk_only=True, avoid_events=True,
    ...
    if u_price*100 > ACCOUNT_BALANCE / 2:
        print(f'Warning! Underlying asset price exceeds account risk tolerance! {u_price*100} > {ACCOUNT_BALANCE/2}')
        if strict_mode:
            return
...
def get_option_chain(yf_obj):
    ...
    for expiration in expirations:
        option_chain[expiration] = ticker_data.option_chain(expiration)
        # todo: calc greeks and add them
